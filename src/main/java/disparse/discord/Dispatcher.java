...
public class Dispatcher extends ListenerAdapter implements
                                                Helpable<MessageReceivedEvent> {
  ...
  private static final Comparator<CommandFlag> helpCommandFlagComparator = new Comparator<>() {
    private Comparator<CommandFlag> comparator =
        Comparator.comparing(this::shortNameKeyExtractor,
  ...
  public void commandNotFound(MessageReceivedEvent event, String userInput) {
    event.getChannel()
         .sendMessage("`" + userInput + "` is not a valid command!").queue();
    event.getChannel()
         .sendMessage("Use !help to get a list of all available commands.")
         .queue();
  }
  ...
  public void roleNotMet(MessageReceivedEvent event, Command command) {
    event.getChannel()
         .sendMessage(
             "You do not have the correct permissions to run:  `" + command
                                                                        .getCommandName()
                 + "`")
         .queue();
  }
  ...
  public void help(MessageReceivedEvent event, Command command,
                   Collection<CommandFlag> flags,
                   Collection<Command> commands) {
    EmbedBuilder builder = new EmbedBuilder();
    builder.setTitle(String.format("%s:  %s", command.getCommandName(),
                                   command.getDescription()))
           .setDescription(String.format(
               "Usage of command:  %s.  [+] may be repeated.",
               command.getCommandName()));

    List<CommandFlag> sortedFlags = flags.stream()
                                         .sorted(helpCommandFlagComparator)
    ...
    for (Command subCommand : subcommands) {
      builder.addField(subCommand.getCommandName(), subCommand.getDescription(),
                       false);
    }
    ...
    for (CommandFlag flag : sortedFlags) {
      ...
      if (flag.getShortName() == null) {
        ...
        flagName = String.format("-%s | --%s", flag.getShortName(),
                                 flag.getLongName());
      }
  ...
  private List<Command> getHelpSubCommands(final Command command,
                                           Collection<Command> commands) {
  ...
  public void allCommands(MessageReceivedEvent event,
                          Collection<Command> commands) {
    EmbedBuilder builder = new EmbedBuilder();
    ...
    List<Command> sortedCommands = commands.stream()
                                           .sorted(Comparator.comparing(
                                               (Command cmd) -> cmd.getCommandName()
                                                                   .toLowerCase(),
